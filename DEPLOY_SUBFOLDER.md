...
在宝塔面板 → **网站** → **starterhunt.asia** → **设置** → **配置文件**

**使用子路径配置（`nginx.conf.subfolder.example`）：**
...
如果你希望直接访问 `http://starterhunt.asia/` 就能看到网站：
...
**`frontend/vite.config.js` 保持默认（不需要 base）：**
...
**使用根路径配置（`nginx.conf.root.example`）：**
